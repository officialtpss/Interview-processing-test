...
const Square = (props, index) => (<button className="square" onClick={() => props.onClick()}>
  {/* eslint-disable-next-line jsx-a11y/alt-text */}
  <img src={props.element} key={index} style={{ display: props.display }} />
</button>);
...
class Board extends Component {
  ...
  getMarioRange() {
    const marioLoc = this.state.marioLocation;
    let marioRange = [];
  ...
  decideMove(enemyLocations) {
    ...
    const marioRange = this.getMarioRange();
    if (
  ...
  checkForEnemies() {
    const enemies = this.state.squares.filter(square => {
      return square.element === 'enemy.png' && square.display === 'block';
    });
    if (enemies.length === 0) {
  ...
  renderBoard() {
    ...
    for (let i = 0, squareNumber = 0; i < this.props.height; i++) {
      ...
      board.push(this.renderRows(rows, i));
      rows = [];
    }
    return board;
  }
    ...
    if (!this.state.gameInitialized) {
      ...
      for (let i = 0; i < this.props.height; i++) {
        for (let j = 0; j < this.props.width; j++) {
          ...
          newData.push({ element, display, value: squareNumber });
          squareNumber++;
        }
      }
      if (!this.state.squares.length) {
        // eslint-disable-next-line react/no-direct-mutation-state
        this.state.squares = newData;
      }
